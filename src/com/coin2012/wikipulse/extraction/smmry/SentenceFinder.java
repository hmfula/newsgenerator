...
public class SentenceFinder {
	 ...
	 * 
	 * @param pageContent
	 *            the text that represents the page content
	 * @param wikiEditContent
	 *            text that represents the edited part of the page content.
	 */
	public static String summarize(final String pageContent,
		...
		if (!editIndexSentenceMap.isEmpty() && !pageIndexSentenceMap.isEmpty()
				&& editIndexSentenceMap.size() >= 1) {
				...
				if (edit != null) {
					for (Entry<Integer, String> entry : pageIndexSentenceMap
						...
						if (entry.getValue().equals(edit)) {
							Integer key = entry.getKey();
							sentenceList
									.add(pageIndexSentenceMap.get(key + 1) != null ? pageIndexSentenceMap
											.get(key + 1) : "");
							break;
						}
					}
